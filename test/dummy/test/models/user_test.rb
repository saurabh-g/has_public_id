...
class UserTest < ActiveSupport::TestCase
  ...
  test 'find_by_public_id' do
    ...
    assert_equal u, User.find_by(public_id: u.to_param), "Can't be looked up by #{u.to_param}"
    assert_nil User.find_by(public_id: 'bad_key'), 'returns nil if not found'
  end

  test 'find_by_public_id!' do
    u = User.create(name: 'joey')
    assert_equal u, User.find_by!(public_id: u.to_param), "Can't be looked up by #{u.to_param}"
    assert_raises(ActiveRecord::RecordNotFound) { User.find_by!(public_id: 'bad_key') }
    assert_raises(ActiveRecord::RecordNotFound) { User.find_by!(public_id: nil) }
  end
